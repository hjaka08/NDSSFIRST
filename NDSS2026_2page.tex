...
\begin{itemize}
  ...
  *attacker does not have to get their ONU/ONT linked with OLT so if attacker remove Tx function of optic module, they can entirely record downstream records without being exposed. 
  \item \textbf{2. Capture GEM} then, attacker extract GTC frames from downstream traffic and they get many GEM frames. 
...
At first we made packet capture datas (PCAPs) of encrypted video traffic of youtube.  we played 10 videos in our restirected ubuntu computer where SSH, VNC traffic is blocked to minimize 간섭. and we simulated that ehternet packet capture file to records the resulting PLI values and timestamps into CSV traces. 
