...
Each GEM header includes a 12 bit payload length indicator (PLI) that specifies the payload length $L$ in bytes (up to 4095 bytes). In certain cases, larger payload values are fragmented accordingly~\cite{itu-g9843}(where PTI is used to indicate whether a GEM payload is a end of a fragment or not).
Downstream GEM payloads are often encrypted using AES in CTR mode (XOR-based encryption).
Therefore, even when payload bytes are encrypted, an attacker can observe the plaintext payload length per fragment via value of PLI.
