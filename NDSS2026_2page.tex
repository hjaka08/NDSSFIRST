...
Since prior studies primarily focused on upper-layer or cellular traffic analysis~\cite{bae-sec22}~\cite{ytyt}, only a few studies focused on optical networks, particularly PON networks which is primarily used for Fiber Internet for home. Inspired by prior studies, we researched whether the plaintext PLI field in GPON GEM header can be used to leak victim's privacy on the same optical splitter.
...
Using these PLI traces, we present preliminary evidence that Youtube streaming video IDs can be identified by attacker.
...
\begin{itemize}
  ...
  *attacker does not have to get their ONU/ONT linked with OLT so if attacker remove Tx function of optic module, they can entirely record downstream records without being exposed.
  ...
  \item \textbf{Ethics:} We used software-based GEM Simulator which makes GEM packet from ehternet packet files. 
\end{itemize}
...
\textbf{Simulator Pipeline}
\begin{enumerate}\setlength\itemsep{0pt}
  \item \textbf{Capture:} For each target video $v$ and run $r$, automate playback and record a PCAP trace $p_{v,r}$. 
  \item \textbf{Simulate GPON:} Parse $p_{v,r}$ and fragment Ethernet frames into GEM units; for each unit, record plaintext \texttt{PLI} and \texttt{t\_start\_us} into \texttt{run$r$.gems.csv}.
  \item \textbf{Build dataset:} Repeat Step 2 for all runs/videos (parallel + resume) to obtain a trace database of \texttt{*.gems.csv}.
  \item \textbf{Feature:} Convert each \texttt{gems.csv} into a dense time series by binning PLI into fixed time bins (e.g., 10\,ms).
  \item \textbf{Train/Test split:} Train a classifier on template runs (e.g., run1--run2) and evaluate on a disjoint run (e.g., run3) for video-ID inference.
\end{enumerate}%
...
Our results indicate that payload encryption may be insufficient to ensure traffic confidentiality in GPON networks. 
Although payload contents are encrypted, plaintext fields such as PLI in GEM headers can be used to infer a victim’s video activity(e.g., Youtube). 
Mitigations may include padding/traffic shaping to dampen length signatures and metadata-aware protocol designs that reduce or obfuscate length exposure at the access layer. 
Future work includes validating our observations on a real GPON testbed. 
All code and experimental data is publicly available on the our github repository.~\cite{github}
...
We expect that this infering methodology is not limited to GPON, but cal anslo We expect that the proposed inference approach is not limited to GPON, but can also extend to XGS-PON networks. XGS-PON preserves the same downstream broadcast architecture and GEM framing format as GPON, and similarly exposes plaintext header fields such as the Payload Length Indicator (PLI) for operational purposes. As a result, an attacker observing downstream transmissions under the same splitter may obtain comparable payload length traces that enable similar traffic inference. While our current evaluation focuses on GPON, we believe that XGS-PON deployments are subject to analogous privacy risks, which we leave for future investigation.
