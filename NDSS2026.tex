...
% in the abstract
\begin{abstract}
Gigabit Passive Optical Networks (GPON) widely protect user traffic by encrypting downstream payloads at the GPON Encapsulation Method (GEM) layer. However, encryption does not cover all access-layer metadata. In this work-in-progress poster, we provide the first evidence that the plaintext Payload Length Indicator (PLI) in GEM headers can form a practical privacy side channel for video streaming inference. We consider an attacker co-located under the same optical splitter who passively observes downstream GPON transmissions without decrypting payloads. Due to GPON’s downstream broadcast and multiplexing, the attacker can obtain a high-rate time series of GEM payload lengths aligned with access-layer framing, which preserves the structural characteristics of HTTP adaptive streaming (HAS), such as segment-driven bursts and bitrate-dependent size patterns.
To study this leakage systematically, we build a GPON downstream trace generator that maps legitimate encrypted video traffic (e.g., YouTube/Netflix sessions) into GEM frames and aggregates them into 125-µs GPON transmission frames, enabling controlled evaluation of observability at the GEM layer. Using derived PLI sequences only, we demonstrate preliminary separability between streaming and non-streaming activity, and we outline how adaptive bitrate transitions can be detected from length dynamics. Our findings highlight that payload encryption alone may be insufficient to protect behavioral privacy in access networks, motivating mitigations such as padding/shaping and privacy-aware handling of access-layer metadata.
\end{abstract}
