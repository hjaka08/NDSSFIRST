...
\begin{itemize}
  ...
  \item \textbf{*} The attacker does not need to register their ONU/ONT on OLT; so even if they use receive-only optical tap, they can entirely record downstream records without transmitting.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.42\linewidth]{fig/cumsum_vid001.pdf}
  \vspace{-0.6em}
  \caption{PLI - Time graph for one of 10 videos showing that highly identical slopes between three runs. PLI - Time Graph of others is publicly available on our github repository~\cite{github}.}
  \label{fig:cumsum}
\end{figure}
...
\begin{figure}[H]
  ...
  \end{minipage}
  \caption{Same video Pair(intra) - Different video pair(inter) similarity yielded from cumulative PLI traces. Same video pairs have smaller distances than different video pairs. We found a partial intersection due to ABR variability and buffering, but the distinction is sufficient for discrimination}
  \label{fig:pli-metrics}
\end{figure} We further quantify the similarity between traces using distance-based metrics on the normalized cumulative PLI time series. Same-video pairs yield smaller distances than different-video pairs Same-video pairs yield smaller distances than different-video pairs (median RMSE: 0.040 vs. 0.088, AUC=0.813; median DTW: 0.031 vs. 0.061, AUC=0.776).
...
\textbf{Extent to XGS-PON Networks.} We assume that XGS-PON are subject to analogous privacy risks because XGEM Frame that XGS-PON uses has Payload Length Indicator Field like GEM.
...
\bibitem{bae-sec22}
S.~Bae, M.~Son, D.~Kim, C.~J.~Park, J.~Lee, S.~Son, and Y.~Kim,
...
\bibitem{ytyt}
X.~Zhang, G.~Xiong, Z.~Li, C.~Yang, X.~Lin, G.~Gou, and B.~Fang,
...
\bibitem{ytyt2}
N.~Hu, H.~Wu, H.~Zhao, S.~Ni, and G.~Cheng,
